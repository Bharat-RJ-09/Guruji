...
// ✅ Added 'avatar' to query
$stmt = $conn->prepare("SELECT full_name, username, email, subscription_plan, created_at, avatar, telegram_chat_id FROM users WHERE id = ?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();

if ($row = $result->fetch_assoc()) {
    if(empty($row['subscription_plan'])) $row['subscription_plan'] = 'free';
    if(empty($row['avatar'])) $row['avatar'] = 'av1'; // Default
